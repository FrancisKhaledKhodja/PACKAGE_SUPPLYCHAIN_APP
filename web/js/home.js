  ...
  async function handleAssistantQuestion() {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (target) {
          const url = new URL(window.location.origin + "/" + target.replace(/^\/+/, ""));
          const codeArticle = params.code_article || params.code || "";

          if (target === "stock.html" && codeArticle) {
            url.searchParams.set("code", codeArticle);
          }
          if (target === "items.html") {
            if (codeArticle) url.searchParams.set("code", codeArticle);
            if (params.q) url.searchParams.set("q", params.q);
          }
          if (target === "photos.html" && codeArticle) {
            url.searchParams.set("code", codeArticle);
          }
          if (target === "helios.html" && codeArticle) {
            url.searchParams.set("code", codeArticle);
          }
          if (target === "stock_map.html") {
            if (params.address) url.searchParams.set("address", params.address);
            if (codeArticle) url.searchParams.set("code", codeArticle);
          }
          if (target === "stores.html" && (params.address || params.q)) {
            url.searchParams.set("q", params.address || params.q);
          }
          if (target === "article_network.html" && codeArticle) {
            url.searchParams.set("code", codeArticle);
          }
          if (target === "statistiques_sorties.html" && codeArticle) {
            url.searchParams.set("code", codeArticle);
          }
          if (target === "technician.html" && (params.q || params.store)) {
            url.searchParams.set("q", params.q || params.store);
          }
          if (target === "technician_assignments.html") {
            if (params.q) url.searchParams.set("q", params.q);
            ...
            if (Array.isArray(params.roles)) {
              params.roles.forEach(r => url.searchParams.append("roles", r));
            }
            if (params.expand_store_roles) {
              url.searchParams.set("expand_store_roles", "1");
            }
          }
