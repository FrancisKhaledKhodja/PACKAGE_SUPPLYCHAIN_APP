...
def get_datagouv_response_gps_for_address(address: str) -> dict:
    ...
    # Otherwise, if no proxy configured yet but proxy_tdf exists, use unauthenticated proxy
    elif proxy_tdf and not proxy_from_env:
        proxy_url = f"http://{proxy_tdf}"
        os.environ["http_proxy"] = proxy_url
        os.environ["https_proxy"] = proxy_url
        proxy_from_env = proxy_url

    proxies = {"http": proxy_from_env, "https": proxy_from_env}
    results = requests.get(URL_DATA_GOUV, params=params, proxies=proxies).json()
