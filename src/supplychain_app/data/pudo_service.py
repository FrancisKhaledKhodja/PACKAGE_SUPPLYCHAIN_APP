...
def get_stock_map_for_all_stores_by_type(
    ref_lat: float | None = None,
    ref_lon: float | None = None,
    type_de_depot_filters: list[str] | None = None,
) -> list[dict]:
    """Retourne les magasins ayant du stock (tous articles confondus),
    agrégés par magasin, en appliquant uniquement un filtre éventuel sur
    le type de dépôt.

    Utilisé pour le mode "sans code article" de la carte de localisation
    du stock. Dans ce mode, les critères de qualité, transit et flag
    D/M ne sont pas pris en compte.
    """
    rows: list[dict] = []
    try:
        # Il faut au minimum le référentiel magasins
        if "stores" not in globals() or stores.is_empty():
            return rows

        # Point de départ : tous les magasins (stores)
        stores_df = stores

        # Ne conserver que les magasins avec statut == 0 si la colonne existe
        if "statut" in stores_df.columns:
            try:
                stores_df = stores_df.with_columns(
                    pl.col("statut").cast(pl.Int64, strict=False).alias("_statut_int")
                ).filter(pl.col("_statut_int") == 0)
            except Exception:
                # En cas de problème de cast, on préfère ne rien filtrer sur le statut
                pass

        # Filtre optionnel sur le type de dépôt uniquement (sur le référentiel magasins)
        if type_de_depot_filters:
            if "type_de_depot" in stores_df.columns:
                stores_df = stores_df.filter(pl.col("type_de_depot").is_in(type_de_depot_filters))

        if stores_df.is_empty():
            return rows

        # Préparer l'agrégat de stock par magasin / type_de_depot si stock_554 est disponible
        stock_agg = None
        if "stock_554" in globals() and not stock_554.is_empty():
            df_stock = stock_554
            required_cols = {"code_magasin", "type_de_depot", "qte_stock"}
            if required_cols.issubset(df_stock.columns):
                df_stock = df_stock.with_columns(
                    pl.col("qte_stock").cast(pl.Float64, strict=False).fill_null(0.0).alias("qte_stock_float")
                )
                # On peut garder uniquement les quantités strictement positives
                df_stock = df_stock.filter(pl.col("qte_stock_float") > 0.0)

                if not df_stock.is_empty():
                    stock_agg = (
                        df_stock.group_by("code_magasin", "type_de_depot")
                        .agg(pl.col("qte_stock_float").sum().alias("qte_stock_total"))
                    )

        # Sélection des colonnes magasins utiles pour l'affichage
        store_cols = [
            "code_magasin",
            "libelle_magasin",
            "type_de_depot",
        ...
        store_cols = [c for c in store_cols if c in stores_df.columns]
        stores_sub = stores_df.select(store_cols)

        # Jointure optionnelle avec l'agrégat de stock, en conservant tous les magasins
        if stock_agg is not None:
            joined = stores_sub.join(stock_agg, on=["code_magasin", "type_de_depot"], how="left")
        else:
            joined = stores_sub.with_columns(
                pl.lit(None, dtype=pl.Float64).alias("qte_stock_total")
            )
        ...
        for r in joined.iter_rows(named=True):
            ...
            qte_val = r.get("qte_stock_total")
            row_dict: dict = {
                "code_magasin": r.get("code_magasin"),
                "libelle_magasin": r.get("libelle_magasin"),
                "type_de_depot": r.get("type_de_depot"),
                # Si aucune info de stock n'est disponible, on met 0.0
                "qte_stock_total": float(qte_val) if qte_val is not None else 0.0,
                "adresse1": adr1,
                "adresse2": adr2,
                "code_postal": cp,
                "ville": ville,
